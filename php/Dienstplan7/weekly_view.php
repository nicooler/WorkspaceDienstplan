...
Task:
Zeitaufwand angezeigter Code: 12h
Beschreibung: Eingeloggter Mitarbeiter kann seine Schichten in der aktuellen Woche anschauen. Die Tabelle ist dynamisch und kann über Eintragungen in der DB verändert werden.
-->
...
while($row = mysqli_fetch_array($timeShift)){
	echo '<tr><td class="innerTableShiftDescription"><span class="shiftDescriptionTableContent">'.$row["DienstBezeichnung"].'<br>'.$row["StartZeit"].' - '.$row["EndZeit"].'</span></td></tr>';
}
	echo '</table></td>';
...
for($i=1; $i<= 7; $i++){
	...
	echo '<table class="dailyShift" cellspacing="0" cellpadding="0">';
		while($tempSc != 0){ #Rangfolge muss abgefragt werden und auch unten im select verwendet werden.
			$tempSc--;
			$sql = "SELECT dp.Rangfolge, dp.StartDatum FROM MitarbeiterArbeiten ma JOIN Dienstplan dp ON dp.PlanID = ma.PlanID WHERE ma.MA_Id = '$maID' AND dp.StartDatum = '$tempDate' AND dp.Rangfolge = '$tempRankShift'";
			$result = mysqli_query($hDB, $sql) or die("Abfrage war nicht erfolgreich");
			
			if(mysqli_fetch_array($result)!=null){
				echo '<tr><td class="innerDailyShiftON"><span class="dailyShiftTableContent">Du hast Schicht !!!</span></td></tr>';
			}else{
				echo '<tr><td class="innerDailyShiftOFF"></td></tr>';
			}
